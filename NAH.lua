repeat task.wait(0.1) until game:IsLoaded() local CONFIG={MAX_RETRIES=10,INITIAL_WAIT=0.3,MAX_WAIT=2.0,EXPONENTIAL_BACKOFF=true,ELEMENT_TIMEOUT=15} local GameState={placeId=nil,gameId=nil,serverId=nil,loadStartTime=nil,isReady=false} local LoaderEngine={} function LoaderEngine:new() local o={elements={},results={successful={},failed={},total=0,completed=0},startTime=nil,isRunning=false} setmetatable(o,{__index=LoaderEngine}) return o end local function updateGameState() GameState.placeId=game.PlaceId GameState.gameId=game.GameId GameState.serverId=game.JobId if GameState.placeId and GameState.gameId and GameState.serverId then GameState.isReady=true GameState.loadStartTime=tick() return true end return false end local function waitForGameState() local e=0 while not GameState.isReady and e<30 do if updateGameState() then return true end task.wait(0.5) e=e+0.5 end return GameState.isReady end local function calcWait(r) local w=CONFIG.INITIAL_WAIT*(1.5^(r-1)) return math.min(w,CONFIG.MAX_WAIT) end function LoaderEngine:loadElement(el) local r=0 local st=tick() while r<CONFIG.MAX_RETRIES do if tick()-st>el.timeout then return false end r=r+1 local s,res=pcall(el.checkFunction) if s and res then el.status="SUCCESS" el.retries=r el.loadTime=tick()-st self.results.successful[el.name]=el self.results.completed+=1 return true end if r>=CONFIG.MAX_RETRIES then el.status="FAILED" el.error=tostring(res) self.results.failed[el.name]=el return false end task.wait(calcWait(r)) end return false end function LoaderEngine:register(n,f,t) t=t or CONFIG.ELEMENT_TIMEOUT table.insert(self.elements,{name=n,checkFunction=f,timeout=t,status="PENDING",retries=0,loadTime=0,error=nil}) self.results.total+=1 end function LoaderEngine:start() if self.isRunning then return false end self.isRunning=true self.startTime=tick() if not waitForGameState() then self.isRunning=false return false end local sc=0 for _,el in ipairs(self.elements) do if self:loadElement(el) then sc+=1 end end self.isRunning=false return sc==self.results.total end local loader=LoaderEngine:new() loader:register("Players Service",function() return game:GetService("Players")~=nil end,5) loader:register("LocalPlayer",function() return game:GetService("Players").LocalPlayer~=nil end,10) loader:register("Character",function() local p=game:GetService("Players").LocalPlayer return p and p.Character~=nil end,15) loader:register("HumanoidRootPart",function() local p=game:GetService("Players").LocalPlayer return p and p.Character and p.Character:FindFirstChild("HumanoidRootPart") end,15) loader:register("Humanoid",function() local p=game:GetService("Players").LocalPlayer return p and p.Character and p.Character:FindFirstChildOfClass("Humanoid") end,10) loader:register("PlayerGui",function() local p=game:GetService("Players").LocalPlayer return p and p:FindFirstChild("PlayerGui") end,10) loader:register("Workspace",function() return game:GetService("Workspace")~=nil end,5) task.spawn(function() loader:start() end) _G.NXLoaderAdv={loader=loader,gameState=GameState,register=function(n,f,t)if not loader.isRunning then loader:register(n,f,t)end end,start=function()return loader:start()end,getStatus=function()return{total=loader.results.total,completed=loader.results.completed,failed=loader.results.total-loader.results.completed}end}
print("NOTHING_X")
task.wait(3)
warn(string.format(
    "Place  | %s | Game  | %s | Server  | %s |",
    game.PlaceId,
    game.GameId,
    game.JobId
))
...
RealRunService = game:GetService("RunService")
RealRunService.Heartbeat:Connect(function(dt)
    for k, v in next, _G.MasterLoopTasks do
        task.spawn(v, dt)
    end
end)
_G.NX_VelCache = {}
_G.NX_PosCache = {}
_G.NX_ClosestTarget = nil
do
	local _nxDeadCbs = {}
	local _nxBackCbs = {}
	local _nxWasDead = false
	local _nxDeathConn = nil
	local function _nxFire(list, ...)
		for i = 1, #list do
			pcall(list[i], ...)
		end
	end
	local function _nxUnreg(list, fn)
		for i = #list, 1, -1 do
			if list[i] == fn then
				table.remove(list, i)
			end
	...
	_G.NX_DEAD = function(fn)
		if type(fn) ~= "function" then
			return function() end
		end
		_nxDeadCbs[#_nxDeadCbs + 1] = fn
		return function()
			_nxUnreg(_nxDeadCbs, fn)
		end
	end
	_G.NX_BACK = function(fn)
		if type(fn) ~= "function" then
			return function() end
		end
		_nxBackCbs[#_nxBackCbs + 1] = fn
		return function()
			_nxUnreg(_nxBackCbs, fn)
		end
	end
	local function _nxBindChar(char)
		if _nxDeathConn then
			pcall(function()
				_nxDeathConn:Disconnect()
			end)
			_nxDeathConn = nil
		end
		if not char then
	...
	_nxLp.CharacterAdded:Connect(_nxBindChar)
end
RealRunService.Heartbeat:Connect(function()
    local velCache = {}
    local posCache = {}
    ...
    for _, p in next, ps:GetPlayers() do
        if p ~= lp and p.Character then
            ...
            if root then
                ...
                velCache[root] = vel
                posCache[root] = root.Position
                if lpRoot then
                    local d = (root.Position - lpRoot.Position).Magnitude
                    if d < bestDist then
                        bestDist = d
                        bestModel = p.Character
                    end
                end
            end
        end
    end
    _G.NX_VelCache = velCache
    _G.NX_PosCache = posCache
    _G.NX_ClosestTarget = bestModel
end)
RealRunService.RenderStepped:Connect(function(dt)
    for k, v in next, _G.MasterRenderTasks do
        task.spawn(v, dt)
    end
end)
RealRunService.Stepped:Connect(function(t, dt)
    for k, v in next, _G.MasterSteppedTasks do
        task.spawn(v, t, dt)
    end
end)
task.spawn(function()
    while true do
        task.wait()
        for k, v in next, _G.MasterYieldingTasks do
            task.spawn(v)
        end
    end
end)
function createFakeEvent(taskTable, originalEvent)
    local FakeEvent = {}
...
FakeHeartbeat = createFakeEvent(_G.MasterLoopTasks, RealRunService.Heartbeat)
FakeRenderStepped = createFakeEvent(_G.MasterRenderTasks, RealRunService.RenderStepped)
FakeStepped = createFakeEvent(_G.MasterSteppedTasks, RealRunService.Stepped)
...
piStreakLabel = nil
piStreakSepObj = nil
function updatePlayerInfoFrame()
...
do
        local _piConn = RealRunService.Heartbeat:Connect(function()
                pcall(updatePlayerInfoFrame)
        end)
        screenGui.Destroying:Connect(function() _piConn:Disconnect() end)
end
...
_nxTitle = Instance.new("TextLabel")
...
keybindPadding = Instance.new("UIPadding")
...
function getSavedControlValue(key)
        if not key or key == "" then
                return nil
        end
        return controlSaveData[key]
end
function setSavedControlValue(key, value)
        if not key or key == "" then
                return
        end
...
targetPadding = Instance.new("UIPadding")
...
infoContainer.Visible = false
infoPadding = Instance.new("UIPadding")
...
windowOutline = Instance.new("Frame")
...
headerDragArea = Instance.new("Frame")
...
sliderStates = {}
controlSaveData = {}
NOTHING_X_UI_SAVE = "NOTHING_X/UI/NOTHING_X_0.file"
NOTHING_X_UI_LOAD = "NOTHING_X/UI/NOTHING_X_0.file"
...
function setTrashBlockEnabled(v)
...
function updateLocalCharacterReferences(newChar)
        char = newChar
        characterSpawnTime = os.clock()
...
isSelectablePlayerDropdownTarget = nil
syncModelDropdownSelectionToManualTarget = nil
stopView = nil
startView = nil
toggleView = nil
...
function getCustomDisplayName(i)
...
worldUpVector = Vector3.new(0, 1, 0)
placesTPs = {
...
safeZoneHPEnabled = false
safeZoneHPSavedCFrame = nil
safeZoneHPInSafeZone = false
safeZoneHPCharacter = nil
safeZoneRestoring = false
safeZoneHPThresholdEnter = 33
safeZoneHPThresholdExit = 37
safeZoneCycleIndex = 0
activeSafeZonePosition = nil
function isSafeZoneActive()
...
function applyTeleportRootState(rootPart, cframe, linearVelocity, angularVelocity, priority)
...
function overpowerRootState(rootPart, cframe, linearVelocity, angularVelocity, priority)
        applyTeleportRootState(rootPart, cframe, linearVelocity, angularVelocity, priority)
end
...
lastHasMainState = nil
lastHasFloorState = nil
floorOnlyPlaces = {
...
function syncSpeedKeybindDisplay()
...
function syncVoidDeadKeybindDisplay()
...
function canSaveSetBackPosition(character, rootPart, humanoid)
...
function toggleVoidDead(state)
        ...
        if targetState == false then
                ...
                if voidDeadLastCF and hrp and humanoid and humanoid.Health > 0 then
                        applyTeleportRootState(hrp, voidDeadLastCF, nil, nil, 4)
                end
                pcall(function()
...
dVoidDeadConn = nil
function toggleDVoidDead(state)
        local targetState = state
        if targetState == nil then
...
function restorePosition(savedCFrame)
        ...
        if hrp and savedCFrame then
                hrp.Anchored = false
                _G.NX_TP(CFrame.new(0, -6666, 0), "SafeZone-Restore", 2)
                hrp.AssemblyLinearVelocity = Vector3.zero
                hrp.AssemblyAngularVelocity = Vector3.zero
                task.wait()
                if hrp.Parent then
                        _G.NX_TP(CFrame.new(savedCFrame.Position.X, -6666, savedCFrame.Position.Z), "SafeZone-Restore", 2)
                        hrp.AssemblyLinearVelocity = Vector3.zero
                        hrp.AssemblyAngularVelocity = Vector3.zero
                        task.wait()
                end
                if hrp.Parent then
                        _G.NX_TP(savedCFrame, "SafeZone-Restore", 2)
                        hrp.AssemblyLinearVelocity = Vector3.zero
                        hrp.AssemblyAngularVelocity = Vector3.zero
                end
...
function toggleAFK(enabled)
        ...
        if afkEnabled then
                ...
                if hrp then
                        hrp.Anchored = false
                        _G.NX_TP(CFrame.new(hrp.Position.X, -6666, hrp.Position.Z), "SafeZoneN-Start", 2)
                        hrp.AssemblyLinearVelocity = Vector3.zero
                        hrp.AssemblyAngularVelocity = Vector3.zero
                end
                local loopIndex = 1
                ...
                afkConnection = RunService.Heartbeat:Connect(function()
                        ...
                        if isFirstTp then
                                ...
                                if afkSavedCFrame then
                                        _G.NX_TP(CFrame.new(afkSavedCFrame.Position.X, -6666, afkSavedCFrame.Position.Z), "SafeZoneN-Loop", 2)
                                else
                                        _G.NX_TP(CFrame.new(root.Position.X, -6666, root.Position.Z), "SafeZoneN-Loop", 2)
                                end
                        else
                                _G.NX_TP(CFrame.new(loopPositions[loopIndex]), "SafeZoneN-Loop", 2)
                ...
                afkCharAddedConnection = player.CharacterAdded:Connect(function(newChar)
                        ...
                        task.spawn(function()
                                ...
                                if newRoot and afkEnabled then
                                        ...
                                        _G.NX_TP(CFrame.new(newRoot.Position.X, -6666, newRoot.Position.Z), "SafeZoneN-Respawn", 2)
                        ...
                        if safeZoneHPInSafeZone then
                                if hp >= 40 then
                                        ...
                                        if isFirstTp then
                                                isFirstTp = false
                                                if safeZoneHPSavedCFrame then
                                                        _G.NX_TP(CFrame.new(safeZoneHPSavedCFrame.Position.X, -6666, safeZoneHPSavedCFrame.Position.Z), "SafeZoneHP-Loop", 2)
                                                else
                                                        _G.NX_TP(CFrame.new(hrp.Position.X, -6666, hrp.Position.Z), "SafeZoneHP-Loop", 2)
                                                end
                                        else
                                                _G.NX_TP(CFrame.new(loopPositions[loopIndex]), "SafeZoneHP-Loop", 2)
                                                loopIndex = (loopIndex % #loopPositions) + 1
                                        end
                                        hrp.AssemblyLinearVelocity = Vector3.zero
                                        hrp.AssemblyAngularVelocity = Vector3.zero
                                ...
                                if hp <= 30 then
                                        ...
                                        hrp.Anchored = false
                                        _G.NX_TP(CFrame.new(hrp.Position.X, -6666, hrp.Position.Z), "SafeZoneHP-Start", 2)
                                        hrp.AssemblyLinearVelocity = Vector3.zero
                                        hrp.AssemblyAngularVelocity = Vector3.zero
                                        isFirstTp = false
                                        loopIndex = 1
...
function flyAlongPath(rootPart, fromPos, toPos, speed, runToken, lookTarget)
...
if not _G._NX_LocalDeadBound then
        _G._NX_LocalDeadBound = true
        _G.NX_DEAD(function()
...
lastSelectableModelsUpdate = 0
lastWorkspaceScan = 0
cachedSelectableModels = {}
function getSelectableTargetModels()
        ...
        for _, p in ipairs(Players:GetPlayers()) do
                playerNamesMap[p.Name] = true
                playerNamesMap[p.DisplayName] = true
        ...
        local function scanFolder(folder)
                ...
                for _, model in ipairs(folder:GetChildren()) do
                        if model:IsA("Model") and model ~= currentCharacter and not Players:GetPlayerFromCharacter(model) and not playerNamesMap[model.Name] and not offlinePlayers[model.Name] then
                                local hum = model:FindFirstChildOfClass("Humanoid")
                                local modelRoot = model:FindFirstChild("HumanoidRootPart")
...
function getClosestAlivePlayerTarget()
        ...
        local cached = _G.NX_ClosestTarget
        if cached and cached.Parent and not isTargetBlacklisted(cached, Players:GetPlayerFromCharacter(cached)) then
...
function getAttackTpPlacement(characterRoot, targetModel, modeOverride)
        ...
        local targetVelocity = (_G.NX_VelCache and _G.NX_VelCache[targetRoot]) or targetRoot.AssemblyLinearVelocity
        ...
        if baseLeadTime == 0 then
                ...
                baseLeadTime = ping > 0 and (ping + 0.0125 + (ping * 0.1)) or 0.045
        end
        ...
        pcall(function()
            local trueVel = targetRoot:GetVelocityAtPosition(targetRoot.Position)
            if trueVel.Magnitude > targetVelocity.Magnitude then
                targetVelocity = trueVel
            end
        end)
        if _isFlingVelocity then
                predictedTargetPosition = targetRoot.Position
        else
                local rawPrediction = targetRoot.Position + (targetVelocity * baseLeadTime)
                predictedTargetPosition = Vector3.new(rawPrediction.X, targetRoot.Position.Y + verticalLead, rawPrediction.Z)
        end
...
SeriousModeTrackerEnabled = false
SeriousModeTrackerActive = false
SM_playerState = {}
SM_activeTimers = {}
SM_playerConnections = {}
SM_PlayersAddedConn = nil
...
CharacterCleanupEnabled = false
antiZeroEnabled = false
_nullWanted = false
_noTpWanted = false
ModConnections = {}
_nxDefenseActing = false
_nxDefenseActingUntil = 0
function _nxMarkDefenseActing()
...
lastQActionType = nil
flyLockedPosition = nil
characterMotor6DsByChar = {}
_cleanupDescConn = nil
_cleanupHumanoidConns = {}
function _isBadMover(v)
...
function _isProtectedWeld(v, char)
...
function _handleNewDesc(v, char)
...
function _setupHumanoidEvents(char)
...
function _bindCleanupEvents(char)
...
antiGrabZeroEnabled = false
_grabAnimConns = {}
_grabCharConn = nil
_grabCharAddedConn = nil
_grabZeroCharAddedConn = nil
function _isGrabAnim(name)
...
function _killGrabTrack(track, char)
        pcall(function() track:Stop(0) end)
...
function _setupGrabOnChar(char)
...
function _noTpSoftStop()
    if _noTpHeartbeat then pcall(function() _noTpHeartbeat:Disconnect() end); _noTpHeartbeat = nil end
    _noTpLastCF = nil
end
function _noTpStart()
    _noTpSoftStop()
    if not _noTpWanted then
        _noTpEnabled = false
        return
    end
    _noTpEnabled = true
    ...
    local rootPart = humanoid.RootPart or char:FindFirstChild("HumanoidRootPart")
    if not rootPart then return end
    ...
    _noTpHeartbeat = game:GetService("RunService").Heartbeat:Connect(function()
        if not _noTpWanted or not _noTpEnabled then return end
    ...
    local function hookPart(part)
        ...
        part:GetPropertyChangedSignal("CFrame"):Connect(function()
            if not _noTpWanted or not _noTpEnabled or _noTpStopRevert or rootPart:GetAttribute("IsAttackTP") or _G.BypassNoTp or active or flying then return end
            revertTp()
        end)
...
_antiVoidHeartbeat = nil
_antiVoidLastSafeCF = nil
_antiVoidCharConn = nil
function isPointSafe(pos)
    local rayOrigin = pos
...
function toggleNoTp(state)
    ...
    if _noTpWanted then
        ...
        _noTpCharConn = lp.CharacterAdded:Connect(function(char)
            if not _noTpWanted then return end
            local hum = char:FindFirstChildOfClass("Humanoid") or char:WaitForChild("Humanoid", 10)
...
_jumpCharAddedConn = nil
_jumpHumanoid = nil
noStunJumpEnabled = true
_autoReturnDeathConn = nil
_autoReturnCharConn = nil
_autoReturnCharRemovingConn = nil
_autoReturnLastDeathCF = nil
function _autoReturnStart()
    local lp = game:GetService("Players").LocalPlayer
...
_G.NX_BACK(function(char)
    if not _autoReturnLastDeathCF then return end
...
task.spawn(function()
    ...
    local function bypassDeathCounter()
        ...
        local animator = humanoid and humanoid:FindFirstChildOfClass("Animator")
        local hrp = character and character:FindFirstChild("HumanoidRootPart")
        if not animator or not hrp then
            isProcessingAntiDeath = false
            return
        end
        ...
        repeat
            ...
            pcall(function()
                if hrp and hrp.Parent then
                    _noTpStopRevert = true
                    _noTpLastCF = savedCF
                    _G.NX_TP(CFrame.new(hrp.Position.X, -6666, hrp.Position.Z), "Anti-Death", 2)
                    hrp.AssemblyLinearVelocity = Vector3.zero
                    hrp.AssemblyAngularVelocity = Vector3.zero
                end
            end)
    ...
    local function setupCharacter(char)
        ...
        if _DXConn then pcall(function() _DXConn:Disconnect() end); _DXConn = nil end
        _DXConn = char.DescendantAdded:Connect(function(desc)
            if desc:IsA("Accessory") and (desc.Name == "RagdollSim" or desc.Name == "Ragdoll") then
                task.defer(function()
    ...
    do
        ...
        end, "NoStunJumpEnabled", false, 1/2)
        makeHubTog(rowJump, "_DX", function(v)
            _DXEnabled = v
        end, "_DXEnabled", false, 1/2)
    end
...
syncVoidDeadKeybindDisplay()
syncPlacesKeybindDisplay()
customOffsetFrame = makeControlFrame(215)
...
task.spawn(function()
        ...
        local function updatePlayerOverlay(targetPlayer)
                ...
                local showBillboard = (humanoid.Health > 0) and (espOverlayConfig.showHp or espOverlayConfig.showCharacter or espOverlayConfig.showUltimate or espOverlayConfig.showStreak or showDeathState or showUltedText)
                local billboard = model:FindFirstChild(ESP_BILLBOARD_NAME)
...
if not _G._NX_LocalBackBound then
        _G._NX_LocalBackBound = true
        _G.NX_BACK(function(newChar)
                if lastDeathCFrame then
                        local deathCF = lastDeathCFrame
                        lastDeathCFrame = nil
                        task.delay(0, function()
...
NORMAL_TRANSPARENCY = 0.66
ULT_TRANSPARENCY = 0.55
...
Cooldowns = {}
function RegisterHotbar(Hotbar)
...
chatConnectionPos = nil
chatConnectionSize = nil
chatConnectionParent = nil
screenConnectionSize = nil
function disconnectChatListeners()
...
function startChatInputBarTracking()
                ...
                while true do
                        ...
                        pcall(function()
                                chatInputBar = CoreGui:FindFirstChild("ExperienceChat")
                                        ...
                                        and CoreGui.ExperienceChat.appLayout:FindFirstChild("chatInputRow")
                        end)
